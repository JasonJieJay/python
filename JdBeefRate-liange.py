...
def get_response(url):#获得响应    param url:    return:
	headers={
...
def get_page(html):#获取页码信息    param html:请求之后的响应体信息    return:返回页码
...
def get_info(html):#获取信息    param html:获得请求的响应    return:
	html = html.replace('fetchJSON_comment98vv510(', '').replace(');', '')#将字符串替换成类似json字符串
	result = json.loads(html)#转化成字典
...
def save_info(info):#保存数据    param info:    return:
